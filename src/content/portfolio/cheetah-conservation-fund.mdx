---
title: "Cheetah Conservation Fund"
backgroundColor: "#FFCB62"
textColor: '#000'
previewImage: ../../images/cheetah-conservation-fund/preview-cheetah-conservation-fund.jpg
marqueeImage: ../../images/cheetah-conservation-fund/marquee-cheetah-conservation-fund.jpg
sortOrder: 2
---

import LogoRedesign from '../../components/portfolio/ccf/LogoRedesign';

<IntroBlock>

The Cheetah Conservation Fund is an international organization dedicated to saving cheetahs in the wild.

</IntroBlock>

<TextBlock>

Through [Avidano Digital](/portfolio/avidano-digital), I led the redesign of [cheetah.org](https://cheetah.org).

We build the website with WordPress and Advanced Custom Fields (ACF). Website managers update and create content in a consistent way across the site. They select from a toolkit of layout components like full-width banners, galleries, figures, and video.

</TextBlock>

<LogoRedesign />

<TextBlock>

CCF wished for a new logo to replace its outdated, image-based logo. It was not scalable and did not work as a single color. Because their logo was so recognizable, **a complete redesign was not an option**. Instead, I drew a simple, vector version of the running cheetah. I also adjusted the type to make the logo feel more contemporary.

</TextBlock>

<FigureSingleBlock>

![Hopper The Rabbit](../../images/cheetah-conservation-fund/grid-desktop-cheetah-conservation-fund-01.jpg)
    
</FigureSingleBlock>

<TextBlock>

Working closely with CCF staff, we restructured the content across cheetah.org to make information easy to find with intuitive navigation in the header and footer. The homepage offers a hierarchy of featured content, up-to-date news, and videos. The new layouts present information in a journalistic, readable style.

</TextBlock>

<FigureSingleBlock>

![Hopper The Rabbit](../../images/cheetah-conservation-fund/grid-mobile-cheetah-conservation-fund.jpg)
    
</FigureSingleBlock>

<TextBlock>

Content across cheetah.org is optimized and readable on smaller devices and larger desktops.

</TextBlock>

<FigureSingleBlock>

![Hopper The Rabbit](../../images/cheetah-conservation-fund/grid-desktop-cheetah-conservation-fund-02.jpg)
    
</FigureSingleBlock>

<TextBlock>

The website has an area just for kids! It encourages children and parents to get involed with CCF. To make it stand out on the website, I created a special CCF *kids* logo.

</TextBlock>

<FigureSingleBlock caption="A logo for the Kids pages of cheetah.org">

![Hopper The Rabbit](../../images/logo-collection/ccf-kids.jpg)
    
</FigureSingleBlock>


<TextBlock>

In addition to the CCF *kids* logo, Using fintcool, I created an entire font called **Cheetah Tracks**. We used the web font on headers within the kids section. CCF uses the desktop font occasionally for email and print communications. 

</TextBlock>


<FigureSingleBlock caption="Logo for the Kids pages of cheetah.org">

![Hopper The Rabbit](../../images/cheetah-conservation-fund/cheetah-tracks-font.jpg)
    
</FigureSingleBlock>

<FigureSingleBlock>

![Hopper The Rabbit](../../images/cheetah-conservation-fund/page-ccf-kids.jpg)
    
</FigureSingleBlock>

<TextBlock>

The new Cheetah.org launched in June of 2019. I am so grateful for the opportunity to work with CCF to raise awareness on their efforts to save the cheetah from extinction.

</TextBlock>

<LiveProjectLink linkURL="https://cheetah.org/" />